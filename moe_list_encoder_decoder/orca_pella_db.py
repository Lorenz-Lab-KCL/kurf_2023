...
class OrcaPellaReader:
    ...
    def __init__(self, log_path: str, with_hydrogen: bool = False):
    ...
    def _make_xyz_file(self, atom_coords):
        head = f"{len(atom_coords)}\n"
        file_path = "xyz/" + str(uuid.uuid4()) + ".xyz"
        np.savetxt(file_path, atom_coords, delimiter=" ", fmt="%s", header=head, comments="")
